'use strict';

const crypto = require('crypto');

const ACCOUNT_URL = 'https://account.withings.com';
const API_URL = 'https://wbsapi.withings.net';

// Notification categories (appli) used by this app.
const APPLI_SLEEP = 44;
const APPLI_BED_IN = 50;
const APPLI_BED_OUT = 51;

const BED_APPLIS = [APPLI_BED_IN, APPLI_BED_OUT];

/**
 * What a night's summary should carry back.
 *
 * Deliberately short. Withings offers a great deal more, but every extra field
 * is health data stored on someone's Homey, so this is limited to what the app
 * actually displays.
 */
const SUMMARY_FIELDS = [
  'sleep_score',
  'hr_average',
  'rr_average'
];

// Bed events (appli 50/51) require user.sleepevents. The v2/sleep series used
// by the polling fallback needs user.activity — without it that call returns
// 403 Insufficient_scope while the webhook path keeps working fine.
const SCOPES = ['user.info', 'user.metrics', 'user.activity', 'user.sleepevents'];

/**
 * Withings signs a subset of the request parameters, sorted by parameter name
 * and joined with commas, using the client secret as an HMAC-SHA256 key.
 */
function buildSignature(params, clientSecret) {
  const payload = Object.keys(params)
    .sort()
    .map(key => params[key])
    .join(',');

  return crypto.createHmac('sha256', clientSecret).update(payload).digest('hex');
}

/**
 * Withings echoes callback URLs back in `notify list` without guaranteeing the
 * exact spelling we sent: a trailing slash or reordered query parameters are
 * enough to make a naive string compare miss. Missing an existing subscription
 * means resubscribing forever; matching a foreign one means never repairing.
 */
function normalizeCallbackUrl(value) {
  if (!value) return '';

  try {
    const url = new URL(value);
    const path = url.pathname.replace(/\/+$/, '');
    const params = [...url.searchParams.entries()].sort(([a], [b]) => a.localeCompare(b));
    const query = params.map(([key, val]) => `${key}=${val}`).join('&');

    return `${url.protocol}//${url.host.toLowerCase()}${path}${query ? `?${query}` : ''}`;
  } catch {
    return String(value).trim().replace(/\/+$/, '');
  }
}

/** How long any single Withings request may take before it is abandoned. */
const REQUEST_TIMEOUT_MS = 15000;

/** Total attempts per call, so a failing service cannot be hammered. */
const MAX_ATTEMPTS = 3;

/** 429 and 5xx clear on their own; 4xx will answer the same however often asked. */
function isTransientStatus(status) {
  return status === 429 || (status >= 500 && status < 600);
}

/** Exponential, starting at a second: 1s, 2s. */
function backoffMs(attempt) {
  return 1000 * 2 ** (attempt - 1);
}

/**
 * Withings' own advice about when to come back, in seconds or as a date.
 * Capped so a wildly long header cannot park a request for an hour.
 */
function retryAfterMs(headers) {
  const raw = headers && typeof headers.get === 'function' ? headers.get('retry-after') : null;
  if (!raw) return null;

  const seconds = Number(raw);
  const ms = Number.isFinite(seconds) ? seconds * 1000 : Date.parse(raw) - Date.now();

  if (!Number.isFinite(ms) || ms < 0) return null;
  return Math.min(ms, 60000);
}

class WithingsError extends Error {
  constructor(status, message) {
    super(`Withings API error ${status}: ${message}`);
    this.status = status;
  }
}

class WithingsApi {
  /**
   * @param {object} options
   * @param {string} options.clientId
   * @param {string} options.clientSecret
   * @param {string} options.redirectUri
   * @param {object} [options.tokens] { accessToken, refreshToken, expiresAt, userId }
   * @param {function} [options.onTokens] Called whenever tokens change, so the
   *   caller can persist them. Receives the new token object.
   * @param {function} [options.fetchImpl] Injectable for tests.
   */
  constructor({
    clientId, clientSecret, redirectUri, tokens = {}, onTokens, fetchImpl,
    timeoutMs = REQUEST_TIMEOUT_MS, sleepImpl
  } = {}) {
    this.clientId = clientId;
    this.clientSecret = clientSecret;
    this.redirectUri = redirectUri;
    this.tokens = { ...tokens };
    this.onTokens = onTokens;
    this.fetch = fetchImpl || globalThis.fetch;
    this.timeoutMs = timeoutMs;
    // Injectable so tests exercise the retry path without waiting for it.
    this._sleep = sleepImpl || (ms => new Promise(resolve => setTimeout(resolve, ms)));
    this._refreshing = null;
  }

  static get APPLI() {
    return {
      SLEEP: APPLI_SLEEP,
      BED_IN: APPLI_BED_IN,
      BED_OUT: APPLI_BED_OUT
    };
  }

  static get BED_APPLIS() {
    return [...BED_APPLIS];
  }

  /**
   * `state` is optional on purpose: Homey's createOAuth2Callback appends its
   * own state to correlate the redirect, so callers inside a pair session
   * should leave it out.
   */
  static buildAuthorizeUrl({ clientId, redirectUri, state }) {
    const query = new URLSearchParams({
      response_type: 'code',
      client_id: clientId,
      scope: SCOPES.join(','),
      redirect_uri: redirectUri
    });

    if (state) query.set('state', state);

    return `${ACCOUNT_URL}/oauth2_user/authorize2?${query.toString()}`;
  }

  /**
   * One HTTP attempt with a deadline.
   *
   * Without a timeout a stalled connection holds a poll or a token refresh
   * open forever, and the interval behind it keeps firing, so the failure
   * compounds instead of surfacing.
   */
  async _fetchWithTimeout(url, options) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), this.timeoutMs);

    try {
      return await this.fetch(url, { ...options, signal: controller.signal });
    } catch (err) {
      if (err && (err.name === 'AbortError' || controller.signal.aborted)) {
        throw new WithingsError(0, `timed out after ${this.timeoutMs} ms`);
      }
      throw err;
    } finally {
      clearTimeout(timer);
    }
  }

  /**
   * POST form-encoded and unwrap Withings' {status, body} envelope.
   *
   * Transport failures are retried, logical ones are not: a rate limit or a
   * server fault clears on its own, while a bad signature returns the same
   * answer however often it is asked. Withings' own Retry-After wins over the
   * local backoff when it sends one.
   */
  async _post(path, params, { attempt = 1 } = {}) {
    let response;

    try {
      response = await this._fetchWithTimeout(`${API_URL}${path}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams(params).toString()
      });
    } catch (err) {
      if (attempt < MAX_ATTEMPTS) {
        await this._sleep(backoffMs(attempt));
        return this._post(path, params, { attempt: attempt + 1 });
      }
      throw err;
    }

    if (isTransientStatus(response.status) && attempt < MAX_ATTEMPTS) {
      const advised = retryAfterMs(response.headers);
      await this._sleep(advised === null ? backoffMs(attempt) : advised);
      return this._post(path, params, { attempt: attempt + 1 });
    }

    if (!response.ok) {
      throw new WithingsError(response.status, `HTTP ${response.status}`);
    }

    const json = await response.json();

    // Withings always returns HTTP 200 and signals failure in the body.
    if (json.status !== 0) {
      throw new WithingsError(json.status, json.error || 'unknown error');
    }

    return json.body || {};
  }

  /**
   * The token endpoint requires a fresh nonce, itself signed with the client
   * secret. Both calls are signed; nothing here uses the access token.
   */
  async _getNonce() {
    const timestamp = Math.floor(Date.now() / 1000);
    const params = {
      action: 'getnonce',
      client_id: this.clientId,
      timestamp
    };

    const body = await this._post('/v2/signature', {
      ...params,
      signature: buildSignature(params, this.clientSecret)
    });

    return body.nonce;
  }

  async _requestToken(grantParams) {
    const nonce = await this._getNonce();
    const signed = {
      action: 'requesttoken',
      client_id: this.clientId,
      nonce
    };

    const body = await this._post('/v2/oauth2', {
      ...signed,
      ...grantParams,
      client_secret: this.clientSecret,
      signature: buildSignature(signed, this.clientSecret)
    });

    return this._storeTokens(body);
  }

  _storeTokens(body) {
    this.tokens = {
      accessToken: body.access_token,
      refreshToken: body.refresh_token,
      // Expire a minute early so a request never races the deadline.
      expiresAt: Date.now() + ((Number(body.expires_in) || 0) - 60) * 1000,
      userId: String(body.userid ?? this.tokens.userId ?? ''),
      // What the user actually granted, which is not always what was asked
      // for. Bed events sit behind user.sleepevents, so a token without it
      // subscribes fine and then never hears a thing. A refresh does not
      // always echo the scope, so keep the last one seen.
      scope: body.scope !== undefined ? String(body.scope) : this.tokens.scope
    };

    if (this.onTokens) this.onTokens(this.tokens);
    return this.tokens;
  }

  /**
   * Prove that the client ID and secret are a valid pair, without needing a
   * user to be authorized. Asking for a nonce is the cheapest call Withings
   * offers that actually verifies the HMAC signature: a wrong secret fails
   * here exactly as it would during a token refresh.
   */
  async verifyCredentials() {
    const nonce = await this._getNonce();
    if (!nonce) throw new WithingsError(0, 'no nonce returned');
    return true;
  }

  async exchangeCode(code) {
    return this._requestToken({
      grant_type: 'authorization_code',
      code,
      redirect_uri: this.redirectUri
    });
  }

  async refresh() {
    // Collapse concurrent refreshes; a burst of calls must not spend the
    // refresh token more than once.
    if (!this._refreshing) {
      this._refreshing = this._requestToken({
        grant_type: 'refresh_token',
        refresh_token: this.tokens.refreshToken
      }).finally(() => {
        this._refreshing = null;
      });
    }

    return this._refreshing;
  }

  async _accessToken() {
    if (!this.tokens.accessToken) throw new Error('Not authorized');
    if (Date.now() >= (this.tokens.expiresAt || 0)) await this.refresh();
    return this.tokens.accessToken;
  }

  /** Authenticated call, retrying once after a forced refresh on 401. */
  async _authed(path, params, { retry = true } = {}) {
    const accessToken = await this._accessToken();

    const response = await this._fetchWithTimeout(`${API_URL}${path}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Authorization: `Bearer ${accessToken}`
      },
      body: new URLSearchParams(params).toString()
    });

    if (isTransientStatus(response.status)) {
      // Same treatment as the unsigned calls, but without recursing through
      // the token logic: one wait, one more attempt, then give up.
      if (retry) {
        const advised = retryAfterMs(response.headers);
        await this._sleep(advised === null ? backoffMs(1) : advised);
        return this._authed(path, params, { retry: false });
      }
      throw new WithingsError(response.status, `HTTP ${response.status}`);
    }

    const json = await response.json();

    if (json.status === 401 && retry) {
      await this.refresh();
      return this._authed(path, params, { retry: false });
    }

    if (json.status !== 0) {
      throw new WithingsError(json.status, json.error || 'unknown error');
    }

    return json.body || {};
  }

  /**
   * Subscriptions are per-category and independent: losing appli 50/51 while
   * appli 1 keeps working is exactly the failure this app exists to avoid.
   */
  async subscribe(callbackUrl, appli, comment = 'Homey') {
    return this._authed('/notify', {
      action: 'subscribe',
      callbackurl: callbackUrl,
      appli,
      comment
    });
  }

  async listSubscriptions(appli) {
    const body = await this._authed('/notify', {
      action: 'list',
      ...(appli === undefined ? {} : { appli })
    });

    return body.profiles || [];
  }

  async revoke(callbackUrl, appli) {
    return this._authed('/notify', {
      action: 'revoke',
      callbackurl: callbackUrl,
      appli
    });
  }

  /**
   * Ensure every category in `applis` is subscribed for this callback URL.
   * Returns the categories that were (re)created.
   */
  async ensureSubscriptions(callbackUrl, applis = BED_APPLIS) {
    const created = [];
    const wanted = normalizeCallbackUrl(callbackUrl);

    for (const appli of applis) {
      const existing = await this.listSubscriptions(appli);
      const active = existing.some(
        profile => normalizeCallbackUrl(profile.callbackurl) === wanted);

      if (!active) {
        await this.subscribe(callbackUrl, appli);
        created.push(appli);
      }
    }

    return created;
  }

  /**
   * Body measurements for the authorized profile, newest first.
   *
   * Withings returns each value as an integer plus a power of ten, so a
   * weight of 84.3 kg arrives as value 84300, unit -3. Decoded here, so the
   * caller sees kilograms. Type 1 is weight, 6 fat ratio, 5 fat-free mass,
   * 8 fat mass, 76 muscle mass, 77 hydration, 88 bone mass.
   *
   * @param {object} [opts]
   * @param {number[]} [opts.types] Measure types to ask for.
   * @param {number} [opts.sinceSeconds] Only groups updated after this epoch.
   */
  async getMeasures({ types = [1, 6, 5, 8, 76, 77, 88], sinceSeconds } = {}) {
    const body = await this._authed('/measure', {
      action: 'getmeas',
      meastypes: types.join(','),
      category: 1,
      ...(sinceSeconds ? { lastupdate: sinceSeconds } : {})
    });

    return (body.measuregrps || [])
      .map(group => ({
        date: Number(group.date),
        deviceId: group.deviceid ? String(group.deviceid) : null,
        values: Object.fromEntries((group.measures || []).map(m => [
          Number(m.type),
          Number(m.value) * 10 ** Number(m.unit)
        ]))
      }))
      .sort((a, b) => b.date - a.date);
  }

  /**
   * Every device on the authorized Withings profile. The mat a notification
   * comes from must be one of these, so an empty sleep-monitor list is the
   * clearest available sign that the user authorized the wrong profile.
   */
  async getDevices() {
    const body = await this._authed('/v2/user', { action: 'getdevice' });
    return body.devices || [];
  }

  /**
   * Completed sleep sessions, one per night, as Withings has scored them.
   *
   * Different endpoint and different date format from getSleepSeries: this one
   * takes calendar days, not epochs, and only returns a night once Withings
   * has finished processing it. That is why it can fill in last night at
   * startup when the raw series is already empty.
   *
   * @param {string} startYmd YYYY-MM-DD
   * @param {string} endYmd YYYY-MM-DD
   */
  async getSleepSummary(startYmd, endYmd, dataFields = SUMMARY_FIELDS) {
    const body = await this._authed('/v2/sleep', {
      action: 'getsummary',
      startdateymd: startYmd,
      enddateymd: endYmd,
      data_fields: dataFields.join(',')
    });

    return body.series || [];
  }

  async getSleepSeries(startDate, endDate) {
    const body = await this._authed('/v2/sleep', {
      action: 'get',
      startdate: startDate,
      enddate: endDate,
      data_fields: 'hr,rr'
    });

    return body.series || [];
  }
}

/**
 * The sleep mats among a profile's devices. Withings reports the 2018 Sleep
 * and the 2020 Sleep Analyzer alike as type "Sleep Monitor", so the type is
 * the thing to match, not the model.
 */
function sleepMonitors(devices) {
  return (devices || []).filter(d => /sleep/i.test(String(d && d.type)));
}

/**
 * The shape of a profile's device list, for the log. Types only, never ids:
 * the point is to tell "this profile owns nothing" from "this profile owns a
 * scale and a blood pressure monitor but not the mat", which call for
 * different fixes on the user's side.
 */
function summariseDevices(devices) {
  const list = devices || [];
  const counts = {};

  for (const d of list) {
    const type = String((d && d.type) || 'unknown');
    counts[type] = (counts[type] || 0) + 1;
  }

  const types = Object.entries(counts)
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([type, n]) => (n > 1 ? `${type} x${n}` : type));

  return { total: list.length, types, mats: sleepMonitors(list).length };
}

module.exports = {
  WithingsApi,
  sleepMonitors,
  summariseDevices,
  SUMMARY_FIELDS,
  REQUEST_TIMEOUT_MS,
  isTransientStatus,
  retryAfterMs,
  WithingsError,
  buildSignature,
  normalizeCallbackUrl,
  SCOPES
};
