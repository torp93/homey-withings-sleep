 ...
 * as a measured resting pulse of nothing.
 *
 * Snoring was removed in 1.6.1. Withings answers 0 rather than omitting it for
 * mats without snoring detection, so the app could not tell "did not snore"
 * from "never measured", and displayed the first while meaning the second.
 *
...
/**
 * Whether a scheduled poll should spend a request on Withings.
 *
 * Polling is a safety net behind the webhook, and on this hardware it has
 * never caught anything the webhook did not: the raw sleep series is not
 * published in near real time, so a poll during an occupied bed reads empty.
 * At one request a minute per device it is also the overwhelming majority of
 * an installation's API traffic, which is what runs an application into
 * Withings' rate limit long before it runs out of users.
 *
 * So it stays, but only where it can help. A poll is skipped only when the
 * webhook has proved itself by delivering an accepted bed event recently.
 * Everything else keeps polling: an installation without a webhook, one whose
 * webhook has never delivered, and the first poll after start, which is
 * reconciliation rather than observation.
 *
 * @param {object} state
 * @param {boolean} state.hasWebhook Whether a cloud webhook was set up.
 * @param {boolean} state.reconciled Whether the startup poll has already run.
 * @param {?number} state.lastEventMs When a bed event was last accepted.
 * @param {number} state.nowMs
 * @param {number} state.trustedMs How long an event vouches for the webhook.
 * @returns {boolean}
 */
function shouldPoll({ hasWebhook, reconciled, lastEventMs, nowMs, trustedMs }) {
  if (!reconciled) return true;
  if (!hasWebhook) return true;
  if (!lastEventMs) return true;

  // Silence outlasting the window is the webhook failing quietly, which is the
  // one case the safety net exists for. A day and a bit covers a normal night
  // plus a long evening, so an ordinary waking day never trips it.
  return nowMs - lastEventMs >= trustedMs;
}
